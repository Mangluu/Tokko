...
function optionalPhoneInput(value, countryCode, localNumber, field) {
  const hasComplete = value !== undefined && value !== null && value !== "";
  const hasLocal = localNumber !== undefined && localNumber !== null && localNumber !== "";
  if (!hasComplete && !hasLocal) return null;
  return phoneInput(value, countryCode, localNumber, field);
}
...
function dependentInput(value, index) {
  ...
  return {
    ...(Number.isInteger(Number(dependent.id)) && Number(dependent.id) > 0
...
function websiteOnboardingInput(body = {}) {
  const dependents = Array.isArray(body.dependents)
    ? body.dependents.map(dependentInput)
    : [];
  if (dependents.length === 0) {
    throw Object.assign(
      new Error("Add at least one family member before continuing"),
      { status: 400 }
    );
  }
  if (dependents.length > 20) {
    throw Object.assign(new Error("A family can contain at most 20 members"), {
      status: 400,
    });
  }
  ...
  if (uniquePhones.size !== dependents.length) {
    throw Object.assign(
      new Error("Each family member must have a different phone number"),
      { status: 400 }
    );
  }
  const primaryParentPhone = optionalPhoneInput(
    body.primaryParentPhone,
    body.primaryParentCountryCode,
    body.primaryParentLocalPhone,
    "primaryParentPhone"
  );
  if (primaryParentPhone && uniquePhones.has(primaryParentPhone)) {
    throw Object.assign(
      new Error("The account phone and a family member phone must be different"),
      { status: 400 }
    );
  }
  return {
    ...
    dependents,
    merchantAuthPhone: null,
    merchantAuthSubjectType: null,
    merchantAuthDependentPhone: null,
    dependentName: dependents[0]?.name || null,
    dependentPhone: dependents[0]?.phone || null,
    dependentRelationship: dependents[0]?.relationshipToUser || null,
  };
}
