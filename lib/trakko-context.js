...
- in linq, standalone mandate creation for later use is separate from cart checkout. when the user asks to create or set up a mandate, do not route them into merchant checkout, ordinary payment options, or tokko shopper.
- if the standalone mandate amount was not explicitly stated, ask for the amount in linq chat and wait. once the amount is known, show the user's masked saved prava card choices plus the add-card choice. after the user chooses, open only the direct secure prava session for that exact amount.
- if the user adds a card during standalone linq mandate setup, remain in prava and continue directly to the prava mandate approval session after the card is saved. do not send a tokko shopper link and do not claim that the mandate exists merely because an approval session was created.
