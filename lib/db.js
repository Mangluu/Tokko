...
async function linkWebsiteUserPhone(
  ...
  try {
    ...
    const heldCards = await client.query(
      `SELECT 1 FROM payment_methods WHERE user_id = $1 LIMIT 1`,
      [userId]
    );
    if (heldCards.rows[0]) {
      throw Object.assign(
        new Error(
          "This parent phone is already linked to another family. Remove your saved card before linking to it."
        ),
        { status: 409 }
      );
    }
  ...
  try {
    ...
    const dependents = Array.isArray(profile.dependents)
      ...
      : profile.dependentPhone
        ...
        : [];
    // Clear the merchant-auth-subject flag on all of this user's rows first, so
    // setting it on the new subject inside the loop can never collide with the
    // old subject (which is only cleared in the post-loop archive step) on the
    // partial unique index family_dependents_merchant_subject_idx.
    await client.query(
      "UPDATE family_dependents SET is_merchant_auth_subject = false WHERE user_id = $1",
      [userId]
    );
      ...
      if (!saved?.rows?.length) {
        saved = await client.query(
          `INSERT INTO family_dependents (
           ...
           ON CONFLICT (user_id, phone) WHERE archived_at IS NULL
           DO UPDATE SET
             name = EXCLUDED.name,
             relationship_to_user = EXCLUDED.relationship_to_user,
             age = EXCLUDED.age,
...
async function recordActivityEvent(userId, event) {
  // The activity feed is a non-essential audit log. It is written after the
  // user operation has already committed on its own connection, so a failure
  // here must never surface as a 500 for a save that actually succeeded.
  try {
    const result = await query(
    ...
    console.error(
      "[db] recordActivityEvent failed (non-fatal):",
      error.message
    );
    return null;
  }
