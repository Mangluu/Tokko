...
async function initialize() {
  ...
  initialization = (async () => {
    ...
    await db.query(
      `ALTER TABLE telegram_hermes_bindings
       ADD COLUMN IF NOT EXISTS pending_mandate JSONB`
    );
    await db.query(
    ...
    await db.query(`
      CREATE TABLE IF NOT EXISTS linq_hermes_bindings (
        chat_id VARCHAR(64) PRIMARY KEY,
        user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    ...
    `);
    await db.query(
      `CREATE INDEX IF NOT EXISTS linq_hermes_bindings_user_idx
       ON linq_hermes_bindings (user_id)`
    );
    await db.query(`
      CREATE TABLE IF NOT EXISTS linq_hermes_messages (
        id BIGSERIAL PRIMARY KEY,
        chat_id VARCHAR(64) NOT NULL
          REFERENCES linq_hermes_bindings(chat_id) ON DELETE CASCADE,
        role VARCHAR(12) NOT NULL CHECK (role IN ('user', 'assistant')),
        content TEXT NOT NULL,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )
    `);
    await db.query(
      `CREATE INDEX IF NOT EXISTS linq_hermes_messages_chat_idx
       ON linq_hermes_messages (chat_id, id DESC)`
    );
    await db.query(`
    ...
    `);
    await db.query(
      `ALTER TABLE ucp_product_choices
     ...
     ON CONFLICT (chat_id) DO UPDATE SET
       ...
       telegram_username = COALESCE(
        ...
        selected_address_id = CASE
          ...
          ELSE NULL
        END,
        pending_mandate = CASE
          WHEN telegram_hermes_bindings.user_id = EXCLUDED.user_id
            THEN telegram_hermes_bindings.pending_mandate
          ELSE NULL
        END,
        updated_at = NOW()
     RETURNING *`,
...
async function saveUcpProductChoices(userId, choices) {
  await query(
    `DELETE FROM ucp_product_choices
     WHERE expires_at <= NOW() OR (user_id = $1 AND created_at < NOW() - INTERVAL '2 hours')`,
    [Number(userId)]
  );
  for (const choice of choices) {
    await query(
      `INSERT INTO ucp_product_choices (
         id, user_id, selection_token, product, expires_at
      ...
      [
        choice.id,
        Number(userId),
        choice.selectionToken,
        JSON.stringify(choice.product || {}),
      ]
    );
  }
}

async function getUcpProductChoice(userId, choiceId) {
  const result = await query(
    `SELECT id, selection_token, product, expires_at
     FROM ucp_product_choices
     WHERE id = $1 AND user_id = $2 AND expires_at > NOW()`,
    [choiceId, Number(userId)]
  );
  return result.rows[0] || null;
}
...
async function getUcpCart(userId) {
  const result = await query(
    `SELECT cart FROM ucp_carts WHERE user_id = $1`,
    [Number(userId)]
  );
  return result.rows[0]?.cart || { items: [] };
}

async function saveUcpCart(userId, cart) {
  const result = await query(
    `INSERT INTO ucp_carts (user_id, cart)
     VALUES ($1, $2::jsonb)
     ON CONFLICT (user_id) DO UPDATE SET
       cart = EXCLUDED.cart,
       updated_at = NOW()
     RETURNING cart`,
    [Number(userId), JSON.stringify(cart || { items: [] })]
  );
  return result.rows[0].cart;
}

async function clearUcpCart(userId) {
  return saveUcpCart(userId, { items: [] });
}

async function getTelegramMandateFlow(chatId, userId) {
  const result = await query(
    `SELECT pending_mandate
     FROM telegram_hermes_bindings
     WHERE chat_id = $1
       AND user_id = $2`,
    [String(chatId), Number(userId)]
  );
  return result.rows[0]?.pending_mandate || null;
}

async function saveTelegramMandateFlow(chatId, userId, flow) {
  const result = await query(
    `UPDATE telegram_hermes_bindings
     SET pending_mandate = $3::jsonb, updated_at = NOW()
     WHERE chat_id = $1
       AND user_id = $2
     RETURNING pending_mandate`,
    [String(chatId), Number(userId), JSON.stringify(flow || {})]
  );
  return result.rows[0]?.pending_mandate || null;
}

async function clearTelegramMandateFlow(chatId, userId) {
  const result = await query(
    `UPDATE telegram_hermes_bindings
     SET pending_mandate = NULL, updated_at = NOW()
     WHERE chat_id = $1
       AND user_id = $2
     RETURNING chat_id`,
    [String(chatId), Number(userId)]
...
async function getLinqHermesBinding(chatId) {
  const result = await query(
    `SELECT binding.*, users.email
     FROM linq_hermes_bindings AS binding
     JOIN users ON users.id = binding.user_id
     WHERE binding.chat_id = $1`,
    [String(chatId)]
  );
...
async function saveLinqHermesBinding(chatId, userId, fromPhone, toPhone) {
  const result = await query(
    `INSERT INTO linq_hermes_bindings (
       chat_id, user_id, from_phone, to_phone
     ) VALUES ($1, $2, $3, $4)
     ON CONFLICT (chat_id) DO UPDATE SET
       user_id = EXCLUDED.user_id,
       ...
       pending_action = CASE
         WHEN linq_hermes_bindings.user_id = EXCLUDED.user_id
           THEN linq_hermes_bindings.pending_action
         ELSE NULL
       END,
       pending_choices = CASE
         WHEN linq_hermes_bindings.user_id = EXCLUDED.user_id
           THEN linq_hermes_bindings.pending_choices
         ELSE NULL
       END,
       updated_at = NOW()
     RETURNING *`,
    [String(chatId), Number(userId), String(fromPhone), String(toPhone)]
  );
  return result.rows[0];
}

async function getLinqHermesMessages(chatId, limit = 24) {
  const result = await query(
    `SELECT role, content, created_at
     FROM (
       SELECT id, role, content, created_at
       FROM linq_hermes_messages
       WHERE chat_id = $1
       ORDER BY id DESC
       LIMIT $2
     ) AS recent
...
async function saveLinqHermesMessage(chatId, role, content) {
  const normalizedRole = role === "assistant" ? "assistant" : "user";
  const normalizedContent = String(content || "").trim().slice(0, 10_000);
  if (!normalizedContent) return null;
  const result = await query(
    `INSERT INTO linq_hermes_messages (chat_id, role, content)
     VALUES ($1, $2, $3)
     RETURNING role, content, created_at`,
    [String(chatId), normalizedRole, normalizedContent]
  );
...
async function saveLinqHermesState(chatId, { pendingAction, pendingChoices }) {
  const result = await query(
    `UPDATE linq_hermes_bindings
     SET pending_action = $2::jsonb,
         pending_choices = $3::jsonb,
         updated_at = NOW()
     WHERE chat_id = $1
     RETURNING pending_action, pending_choices`,
    [
      String(chatId),
      pendingAction ? JSON.stringify(pendingAction) : null,
      pendingChoices ? JSON.stringify(pendingChoices) : null,
    ]
  );
  return result.rows[0] || null;
}
...
async function getLinqFamilyCandidatesByPhone(phone) {
  const result = await query(
    `SELECT DISTINCT users.id, users.email,
            (users.phone = $1 OR profiles.primary_parent_phone = $1)
              AS is_account_owner
     FROM users
     JOIN family_profiles profiles ON profiles.user_id = users.id
     WHERE users.phone = $1
        OR profiles.primary_parent_phone = $1
        OR profiles.secondary_parent_phone = $1
        OR profiles.offspring_phone = $1
        OR EXISTS (
          SELECT 1
     ...
     ORDER BY is_account_owner DESC, users.id`,
    [phone]
  );
  return result.rows;
}
...
async function selectFamilyAddress(userId, addressId) {
  ...
  try {
    ...
    // Serialize address switches for this user so the partial unique index can
    // never observe two selected addresses during concurrent confirmations.
...
async function transitionCheckoutFlow(userId, id, fromStatus, toStatus) {
  const result = await query(
    `UPDATE checkout_flows
     SET status = $4, updated_at = NOW(), failure_message = NULL
     WHERE id = $1
       AND user_id = $2
       AND status = $3
     RETURNING *`,
    [String(id), Number(userId), String(fromStatus), String(toStatus)]
  );
  return result.rows[0] || null;
}
...
async function forgetWebhookEvent(provider, eventId) {
  await query(
...
module.exports = {
  ...
  getLinqFamilyCandidatesByPhone,
  getFamilyAddresses,
  getHermesMemories,
  getLatestMerchantAuthAttempt,
  getLinqHermesBinding,
  getLinqHermesMessages,
  ...
  recordActivityEvent,
  saveLinqAssignment,
  saveLinqHermesBinding,
  saveLinqHermesMessage,
  saveLinqHermesState,
  saveAddressGeocode,
  saveCheckoutFlow,
  saveCareRules,
