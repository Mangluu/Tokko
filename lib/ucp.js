...
// These are the merchant personas available to Hermes. Product discovery is
// intentionally limited to their own live UCP endpoints. It must not broaden
// to Zepto, Shopify Global Catalog, or merchants outside this memory scope.
const HERMES_MERCHANT_SLUGS = Object.freeze([
  "setunutrition",
  "wellbeingnutrition",
  "yogabar",
  "dotandkey",
...
const HERMES_MERCHANTS = Object.freeze(
  Object.fromEntries(HERMES_MERCHANT_SLUGS.map((slug) => [slug, MERCHANTS[slug]]))
);
...
function discoveryCurl(endpoint) {
  return [
    "curl -sS",
    curlQuote(endpoint),
    "-H",
    curlQuote("Accept: application/json"),
  ].join(" ");
}

function mcpCurl(endpoint, requestBody, buyerIp) {
  return [
    "curl -sS",
    curlQuote(endpoint),
    "-H",
    curlQuote("Content-Type: application/json"),
    "-H",
    curlQuote("Accept: application/json, text/event-stream"),
    "-H",
    curlQuote(`MCP-Protocol-Version: ${UCP_VERSION}`),
    ...Object.entries(ucpBuyerIpHeaders(buyerIp)).flatMap(
      ([key, value]) => ["-H", curlQuote(`${key}: ${value}`)]
    ),
    "--data-binary",
    curlQuote(JSON.stringify(requestBody)),
  ].join(" ");
}

function logSearchFailure(details, error) {
  console.error("[ucp:search] request failed", JSON.stringify({
    ...details,
...
function agentProfileUrl(_baseUrl) {
  return String(
...
function hermesMerchantFromHint(value) {
  const hint = String(value || "").trim().toLowerCase();
  if (!hint) return null;
  if (HERMES_MERCHANTS[hint]) return HERMES_MERCHANTS[hint];
  const compact = hint.replace(/[^a-z0-9]/g, "");
  return Object.values(HERMES_MERCHANTS).find((entry) =>
...
// Shopify buyer-IP headers, mirroring prava-demo-monorepo's ucpBuyerIpHeaders.
// Sourced from an explicit buyerIp (threaded from the request) or the UCP_BUYER_IP
// override; omitted when neither is set. Shopify degrades create_checkout (drops
// shipping rates / returns a degraded storefront URL) without it.
function ucpBuyerIpHeaders(buyerIp) {
  const ip = String(buyerIp || process.env.UCP_BUYER_IP || "").trim();
  if (!ip) return {};
  return {
    "Shopify-Buyer-IP": ip,
  ...
  try {
    const response = await fetchImpl(endpoint, {
      ...
      headers: {
        ...
        "MCP-Protocol-Version": UCP_VERSION,
        "UCP-Agent": `profile="${profile}"`,
        ...ucpBuyerIpHeaders(buyerIp),
      },
    ...
    if (failureLog) {
      logSearchFailure({
        ...failureLog,
        endpoint,
        curl: mcpCurl(endpoint, requestBody, buyerIp),
      }, error);
    }
    throw error;
  }
...
async function searchMerchant(
  ...
  let selected;
  try {
    selected = await discover(slug, fetchImpl);
  } catch (error) {
    const configured = merchant(slug);
    logSearchFailure({
      phase: "discovery",
      merchant: configured.slug,
      merchantName: configured.name,
      endpoint: configured.discoveryUrl,
      curl: discoveryCurl(configured.discoveryUrl),
    }, error);
    throw error;
  }
...
async function searchAll(
  ...
  const eligibleMerchants = Object.values(HERMES_MERCHANTS).filter(
  ...
  const merchantLimit = Math.min(
    Math.max(
      Math.floor(Number(merchantResultLimit) || 0),
      pageOffset + pageSize,
      pageSize
    ),
    MAX_RESULTS_PER_MERCHANT
  );
  const targetedMerchant = hermesMerchantFromHint(merchantHint);
  if (merchantHint && !targetedMerchant) {
    throw httpError(400, "Unsupported Hermes UCP merchant");
  }
  if (
    targetedMerchant
    && normalizedMarket
  ...
  const merchantEntries = targetedMerchant
    ? [targetedMerchant]
    : eligibleMerchants;
  ...
  return {
    ...
    searchStrategy,
    queriesTried,
    source: "live_hermes_merchant_ucp",
    selectedMerchant: targetedMerchant?.slug || null,
...
function checkoutInput({ selection, count, lineItems, buyer, destination }) {
  const checkout = {
    currency: selection.currency || "INR",
    line_items: Array.isArray(lineItems) && lineItems.length
      ? lineItems
      : [{ item: { id: selection.variantId }, quantity: count }],
  };
...
async function createCheckout(
  ...
  const requestedItems = Array.isArray(selectionToken)
    ? selectionToken
    : [{ selectionToken, quantity }];
  if (!requestedItems.length || requestedItems.length > 50) {
    throw httpError(400, "Checkout requires between 1 and 50 cart items");
  }
  ...
  const merchantKey = (value) => [
    value.merchant,
    value.merchantOrigin || "",
  ].join("|");
  if (selections.some((item) =>
    merchantKey(item.selection) !== merchantKey(selection)
  )) {
    throw httpError(409, "All checkout items must belong to one merchant");
  }
  if (selections.some((item) =>
    String(item.selection.currency || "").toUpperCase()
      !== String(selection.currency || "").toUpperCase()
  )) {
    throw httpError(409, "All checkout items must use one currency");
  }
  ...
  const count = selections.reduce((total, item) => total + item.count, 0);
  const lineItems = selections.map((item) => ({
    item: { id: item.selection.variantId },
    quantity: item.count,
  }));
  ...
  const quotedItemsMinor = selections.reduce(
    (total, item) =>
      total + Number(item.selection.priceMinor || 0) * item.count,
    0
  );
  const pricing = checkoutPricing(payload, quotedItemsMinor);
  const checkout = checkoutResource(payload);
  ...
  return {
    ...
    productName: selections.length === 1
      ...
      : `${selections.length} cart items`,
    variantName: selections.length === 1 ? selection.variantName : null,
    variantId: selections.length === 1 ? selection.variantId : null,
    quantity: count,
    market: selection.deliveryMarket || selected.market,
    currency: String(checkout?.currency || selection.currency || "INR").toUpperCase(),
    quotedUnitPrice: selections.length === 1
      ? Number(selection.priceMinor || 0) / 100
      : null,
    ...pricing,
    itemsSubtotalMinor: pricing.subtotalMinor || quotedItemsMinor,
