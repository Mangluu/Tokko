...
async function createMandateSession({
  ...
  const mandatePaymentCurrency = String(currency || mandateCurrency())
    .trim()
    .toUpperCase();
  if (!/^[A-Z]{3}$/.test(mandatePaymentCurrency)) {
    throw httpError(400, "Mandate currency must be an ISO 4217 currency code");
  }
  const savedCardId = String(cardId || "").trim();
  const userPhone = String(phone || "").trim();
  const userCountryCode = String(countryCode || "").trim().toUpperCase();
  if (userPhone && !/^\+[1-9]\d{6,14}$/.test(userPhone)) {
    throw httpError(400, "Mandate phone must be an E.164 phone number");
  }
  if (userCountryCode && !/^[A-Z]{2}$/.test(userCountryCode)) {
    throw httpError(400, "Mandate country must be an ISO country code");
  }
  ...
  if (!/^[A-Z]{2}$/.test(merchantCountryCode)) {
    throw httpError(
      503,
      "The configured Prava mandate country must be an ISO country code"
    );
  }
  const defaultPurchaseContext = [
    {
      merchant_details: {
        name: merchantName,
        url: merchantUrl,
        country_code_iso2: merchantCountryCode,
      },
      product_details: [
        {
  ...
  const data = await pravaRequest("/v1/sessions", {
    ...
    body: {
      ...
      currency: mandatePaymentCurrency,
      integration_type: "full_checkout",
      callback_url: callbackUrl,
      ...(savedCardId
        ? {
            card: {
              card_id: providerIdentifier(savedCardId, "Prava card ID"),
            },
          }
        : {}),
      ...(userPhone ? { user_phone: userPhone } : {}),
      ...(userCountryCode
        ? { user_country_code_iso2: userCountryCode }
        : {}),
      external_order_ref: providerIdentifier(
        externalOrderRef || `tokko_care_mandate_${Date.now()}`,
        "Mandate external order reference"
      ),
      description: String(
        description
      ...
      purchase_context: Array.isArray(purchaseContext) && purchaseContext.length
        ? purchaseContext
...
async function createPaymentSession({
  ...
  const savedCardId = String(cardId || "").trim();
  const paymentCurrency = String(currency || mandateCurrency()).trim().toUpperCase();
  if (!/^[A-Z]{3}$/.test(paymentCurrency)) {
    throw httpError(400, "Payment currency must be an ISO 4217 currency code");
  }
  const userPhone = String(phone || "").trim();
  const userCountryCode = String(countryCode || "").trim().toUpperCase();
  if (userPhone && !/^\+[1-9]\d{6,14}$/.test(userPhone)) {
    throw httpError(400, "Payment phone must be an E.164 phone number");
  }
  if (userCountryCode && !/^[A-Z]{2}$/.test(userCountryCode)) {
    throw httpError(400, "Payment country must be an ISO country code");
  }
  const data = await pravaRequest("/v1/sessions", {
    method: "POST",
    body: {
      ...
      // Prava lets the buyer enter a new card on the hosted page when `card`
      // is omitted; only pin to a saved card when we actually have one.
      ...(savedCardId
        ? {
            card: {
              card_id: providerIdentifier(savedCardId, "Prava card ID"),
      ...
      description: String(
        description || "Authorize this card payment with Prava."
      ).trim().slice(0, 500),
