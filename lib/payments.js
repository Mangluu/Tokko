...
async function pravaRequest(
  ...
  if (!response.ok) {
    ...
    throw httpError(
      // A Prava auth or rate-limit error is our integration's fault, not the
      // caller's, so surface it as a gateway error instead of a 401/403/429
      // the client would misread as their own session or rate limit.
...
function safeMandate(value) {
  ...
  return {
    ...
    frequency:
      String(value.recurringFrequency || "one_time").toLowerCase(),
    merchantScope: value.merchantScope || null,
    merchantName: value.merchantName || null,
    approvedAmount: String(value.approvedAmount || "0"),
    remaining: String(value.remaining ?? value.approvedAmount ?? "0"),
    ...
    ...((value.updatedAt || value.updated_at)
      ...
      : {}),
    lastCharge: value.lastCharge
      ? {
          status: value.lastCharge.status || null,
          at: value.lastCharge.at || null,
        }
      : null,
    ...(value.spent !== undefined && value.spent !== null
      ? { spent: String(value.spent) }
      : {}),
    ...(value.chargeCount !== undefined && value.chargeCount !== null
      ? { chargeCount: Number(value.chargeCount) }
      : {}),
    ...(Array.isArray(value.charges)
      ? {
          charges: value.charges.map((charge) => ({
...
async function listMandates(customerId) {
  ...
  return (Array.isArray(data.mandates) ? data.mandates : [])
    .map(safeMandate)
    .filter(Boolean);
}
