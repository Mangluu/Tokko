...
function normalizeMessages(values) {
  ...
  const normalized = values
    ...
    .filter(Boolean);
  // Gemini rejects a history that does not begin with a user turn.
...
- request no more than 50 products. preserve eligible lowest-price ordering within each market and currency. use offset 50, 100, and so on only when the user asks for more.
...
function mcpToolsToGemini(tools) {
  return (Array.isArray(tools) ? tools : [])
    ...
    .map((tool) => {
      ...
      // Gemini rejects an OBJECT schema with empty properties, so a
      // no-argument tool must omit the parameters field entirely.
      const hasNoProperties =
        parameters.type === "object" &&
        parameters.properties &&
        Object.keys(parameters.properties).length === 0;
      if (!hasNoProperties) declaration.parameters = parameters;
      return declaration;
    });
}
      ...
      if (failed) {
        ...
        message =
          completed > 0
            ...
            : `i’m sorry, the order did not complete: ${failures}`;
        ...
        message = `done, ${completed} approved action${completed === 1 ? " is" : "s are"} complete.`;
      }
      return {
