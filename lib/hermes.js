...
function systemPrompt(context = {}) {
  ...
  const family = {
    ...
    lockedMerchant: context.lockedMerchant || null,
    lockedMerchantName: context.lockedMerchantName || null,
...
- if confirmedDeliveryAddress exists, never ask for it again. search with selectedDeliveryCountry as the exact ISO two-letter shipping-country filter.
- if lockedMerchant is present, the cart already belongs to that merchant. use exactly lockedMerchant for every later product search until the cart is cleared or explicitly replaced. do not suggest or search another merchant.
- if lockedMerchant is absent for an IN address, choose exactly one merchant from eligibleMerchants before catalogue search. use checkout_preference, successful_wellness_search, and product_preferences memory first; otherwise choose the best Indian merchant persona for the request. never call every Indian merchant in parallel.
- for every non-IN address, use the global UCP catalogue with ships_to.country set to selectedDeliveryCountry. let live item-level deliverability decide the merchants. do not choose a fixed India merchant or reuse an India-only merchant memory.
- request at most 10 products per item. preserve availability first, then learned past-order and stated preference fit, then live quality signals when returned, then lowest price. use offset 10, 20, and so on only when the user asks for more, and keep using the cart's locked merchant.
- when one request names multiple distinct products or categories, such as hair oil, cough syrup, and shampoo, call search_wellness_merchants separately for each item and return up to 10 preferred options for each. never collapse the items into one combined catalogue query.
- rank merchant choice and options using learned checkout_preference, successful_wellness_search, product_preferences, and cross-merchant order context. a repeated exact product or brand is preferred when it remains available and appropriate. use a quality ranking only when the merchant returns an objective rating or review signal. never invent a quality score.
...
- create_wellness_checkout submits the selected saved address and phone and obtains the merchant's authoritative item, shipping, discount, tax, fee, delivery window, and final merchant total. tokko then adds a 3 percent foreign-exchange charge on that entire merchant total. the breakdown must show that forex value and the final payable total. it creates a quote only.
- never show or mention the merchant checkout url. show the full merchant quote, then ask the user to choose proceed with order or do not place order.
- after the user confirms the quote, present exactly two broad payment categories: mandates and saved cards. under mandates, show whether active mandates cover the full cart and allow mandate creation. under saved cards, show every saved card and allow a new card to be saved. only a prava-hosted approval url may be shown.
- when several active mandates cover the full cart, use the smallest remaining-value mandate that covers the entire amount. never split one order across mandates.
- mandate eligibility and the prava mandate charge must use the final payable amount after the 3 percent forex charge, never the merchant subtotal or pre-forex total.
- cart items may be removed individually, and the whole cart may be emptied, whenever the user asks. once the cart is empty, clear the merchant lock for later searches.
...
async function transcribeAudio({
  ...
  if (!bytes.length || bytes.length > 2_250_000) {
    throw httpError(400, "The voice recording is empty or too large");
  }
    ...
    if (!response.ok) {
      ...
      throw httpError(
        ...
        response.status === 429
          ...
          : `Voice transcription could not process this recording: ${String(
              data?.error?.message || "unsupported audio response"
            ).slice(0, 300)}`
      );
    }
...
const SHOPPING_MEDIA_MIME_TYPES = new Set([
  "application/pdf",
  "image/heic",
  "image/jpeg",
  "image/png",
...
function parseMediaInspection(value) {
  ...
  try {
    ...
    throw httpError(502, "Hermes could not read a structured result from this upload");
  }
  const documentType = ["prescription", "product", "unknown"].includes(parsed?.documentType)
    ? parsed.documentType
    : "unknown";
  const items = (Array.isArray(parsed?.items) ? parsed.items : [])
    .map((item) => ({
      name: String(item?.name || "").trim().slice(0, 300),
      brand: item?.brand ? String(item.brand).trim().slice(0, 160) : null,
      kind: ["prescription_medicine", "non_prescription_product", "unknown"]
        .includes(item?.kind)
        ? item.kind
        : "unknown",
  ...
  return {
    documentType,
    items,
    query: String(parsed?.query || items[0]?.name || "").trim().slice(0, 500),
  };
}

async function inspectShoppingMedia({
  dataBase64,
  mimeType,
  language,
  declaredType = "auto",
  ...
  const config = configuration();
  if (!config.configured) throw httpError(503, "Image understanding is not configured");
  const normalizedMimeType = String(mimeType || "").split(";")[0].trim().toLowerCase();
  if (!SHOPPING_MEDIA_MIME_TYPES.has(normalizedMimeType)) {
    throw httpError(400, "Upload a JPEG, PNG, WebP, HEIC, or PDF file");
  }
  ...
  if (
    ...
    throw httpError(400, "The uploaded file is empty or larger than 2.25 MB");
  }
  const bytes = Buffer.from(encoded, "base64");
  if (!bytes.length || bytes.length > 2_250_000) {
    throw httpError(400, "The uploaded file is empty or larger than 2.25 MB");
  ...
  for (let attempt = 0; attempt < GEMINI_RETRY_ATTEMPTS; attempt += 1) {
    ...
    try {
        ...
        `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(config.model)}:generateContent`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              ...
              parts: [{
                text: [
                  "classify this shopping upload and read only clearly visible text.",
                  `the user declared it as: ${String(declaredType || "auto")}.`,
                  "return JSON only with documentType (prescription, product, or unknown), items, and query.",
                  "each item must have name, brand or null, kind (prescription_medicine, non_prescription_product, or unknown), and confidence from 0 to 1.",
                  "for prescriptions, copy medicine names exactly and do not infer dosage, diagnosis, substitutions, or illegible text.",
                  "for product packets, identify the exact product and brand when visible; query should be the best catalogue search phrase.",
                  `the selected response locale is ${responseLanguage}, but preserve printed product and medicine names.`,
                ].join(" "),
              }, {
                inlineData: { mimeType: normalizedMimeType, data: encoded },
              }],
            }],
            generationConfig: {
              maxOutputTokens: 2_048,
      ...
      if (error.name === "AbortError") throw httpError(504, "Upload analysis took too long");
      throw httpError(502, "Upload analysis could not reach the image service");
    } finally {
      clearTimeout(timeout);
    }
    const data = await response.json().catch(() => ({}));
    ...
    if (!response.ok) {
      throw httpError(
        ...
        response.status === 429
          ? "Upload analysis is briefly busy. Please retry in a few seconds."
          : `Upload analysis failed: ${String(data?.error?.message || "unsupported media response").slice(0, 300)}`
      );
    }
    const text = data?.candidates?.[0]?.content?.parts
      ...
      .join("\n");
    return {
      ...parseMediaInspection(text),
      language: responseLanguage,
      model: config.model,
    };
  }
  throw httpError(502, "Upload analysis could not process this file");
}
...
function deriveLearnedMemories(messages, result) {
  ...
  for (const tool of Array.isArray(result?.tools) ? result.tools : []) {
    ...
    if (
      ...
      const merchants = Array.isArray(tool.result?.products)
        ? [...new Map(tool.result.products
          .filter((product) => product?.merchant)
          .map((product) => [String(product.merchant), {
            slug: String(product.merchant),
            name: String(product.merchantName || product.merchant),
          }])).values()].slice(0, 5)
        : [];
      if (cue && (queries.length || products.length)) {
        memories.push({
          type: tool.name === "search_wellness_merchants"
            ...
            : "successful_search",
          cue,
          value: tool.name === "search_wellness_merchants"
            ? { queries, products, merchants }
            : { queries, products },
