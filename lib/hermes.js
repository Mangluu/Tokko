...
- choose exactly one merchant from eligibleMerchants before catalogue search. use checkout_preference, successful_wellness_search, product_preferences, the approved merchant personas, and the current request. search only that merchant's live UCP. never use Zepto or a global catalogue as fallback.
- request no more than 50 products. preserve availability and live price ordering. use offset 50, 100, and so on only when the user asks for more, and keep using the remembered or selected merchant.
