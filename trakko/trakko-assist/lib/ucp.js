...
function normalizeCountryMarket(value, field = "market") {
  const normalized = String(value || "").trim().toUpperCase();
  if (!/^[A-Z]{2}$/.test(normalized)) {
    throw httpError(400, `${field} must use a two-letter country code`);
  }
  return normalized;
}

function preferredCurrencyForMarket(market) {
...
function merchantFromHint(value) {
  const hint = String(value || "").trim().toLowerCase();
  if (!hint) return null;
  if (MERCHANTS[hint]) return MERCHANTS[hint];
  const compact = hint.replace(/[^a-z0-9]/g, "");
  return Object.values(MERCHANTS).find((entry) =>
    entry.slug.replace(/[^a-z0-9]/g, "") === compact
    || entry.name.toLowerCase().replace(/[^a-z0-9]/g, "") === compact
  ) || null;
}
  ...
  for (const product of Array.isArray(payload?.products) ? payload.products : []) {
    for (const variant of Array.isArray(product?.variants) ? product.variants : []) {
      ...
      const currency = String(
        variant?.price?.currency || selected.currency || ""
      ).trim().toUpperCase();
      ...
      const item = {
        ...
        rating: Number(product?.rating?.value || product?.rating || variant?.rating) || null,
        reviewCount: Number(
          product?.rating?.count || product?.review_count || product?.reviewCount
        ) || null,
      };
...
async function searchGlobalCatalog(
  ...
  const normalizedMarkets = [...new Set(
    (Array.isArray(markets) ? markets : [markets]).map((market) =>
      normalizeCountryMarket(market, "markets")
    )
  )];
  const results = await Promise.allSettled(normalizedMarkets.map(async (market) => {
    const preferredCurrency = preferredCurrencyForMarket(market);
  ...
  return {
    ...
    markets: results.map((entry, index) => ({
      market: normalizedMarkets[index],
...
async function searchGlobalMarket(
  query,
  { limit, offset, market = "US", merchant = null, baseUrl, fetchImpl = fetch } = {}
) {
  const normalizedQuery = String(query || "").trim().slice(0, 300);
  if (!normalizedQuery) throw httpError(400, "query is required");
  const normalizedMarket = normalizeCountryMarket(market || "US");
  const pageSize = Math.min(Math.max(Math.floor(Number(limit) || 3), 1), 50);
  const pageOffset = Math.min(Math.max(Math.floor(Number(offset) || 0), 0), 47);
  const requestedLimit = Math.min(pageOffset + pageSize, 50);
  let globalSearch = await searchGlobalCatalog(normalizedQuery, {
    markets: [normalizedMarket],
    limit: requestedLimit,
    baseUrl,
    fetchImpl,
  });
  const lockedMerchant = String(merchant || "").trim().toLowerCase();
  const keepLockedMerchant = (values) => lockedMerchant
    ? values.filter((product) => String(product.merchant).toLowerCase() === lockedMerchant)
    : values;
  let products = keepLockedMerchant(globalSearch.products);
  ...
  if (!products.length && fallbackQueries.length > 1) {
    ...
    for (const fallbackQuery of fallbackQueries) {
      const fallback = await searchGlobalCatalog(fallbackQuery, {
        markets: [normalizedMarket],
        limit: requestedLimit,
        baseUrl,
        fetchImpl,
      });
      products = mergeProducts([...products, ...keepLockedMerchant(fallback.products)]);
  ...
  for (const product of products) {
    if (!merchantGroups.has(product.merchant)) {
      merchantGroups.set(product.merchant, {
    ...
    merchantGroups.get(product.merchant).resultCount += 1;
  }
  const remotePagination = globalSearch.markets[0]?.pagination || {};
  const remoteHasMore = Boolean(
    remotePagination.has_next_page
    || remotePagination.hasNextPage
    || remotePagination.has_more
    || remotePagination.hasMore
  );
  return {
    ...
    market: normalizedMarket,
    markets: [normalizedMarket],
    sort: "availability_then_lowest_native_price",
    selectedMerchant: lockedMerchant || null,
    products: pageProducts,
    merchants: [...merchantGroups.values()],
    pagination: {
      offset: pageOffset,
      limit: pageSize,
      ...
      hasMore:
        ...
        || (
          ...
          && requestedLimit < 50
          && (remoteHasMore || products.length >= requestedLimit)
        ),
    },
  };
}
...
function normalizeProducts(selected, payload, query, matchMode = "all") {
  ...
  for (const product of products) {
    ...
    for (const variant of variants) {
      ...
      item.selectionToken = signSelection({
        merchant: item.merchant,
        deliveryMarket: selected.market,
        variantId: String(variant.id),
        productName: item.productName,
        variantName: item.variantName,
        priceMinor: item.priceMinor,
...
async function searchAll(
  query,
  { limit, offset, market, merchant: merchantHint, baseUrl, fetchImpl = fetch } = {}
) {
  const normalizedQuery = String(query || "").trim().slice(0, 300);
  if (!normalizedQuery) throw httpError(400, "query is required");
  const normalizedMarket = String(market || "").trim().toUpperCase();
  if (normalizedMarket) normalizeCountryMarket(normalizedMarket);
  ...
  const targetedMerchant = merchantFromHint(merchantHint);
  if (merchantHint && !targetedMerchant) {
    throw httpError(400, "Unsupported UCP merchant");
  }
  if (
    targetedMerchant
    && normalizedMarket
    && targetedMerchant.market !== normalizedMarket
  ) {
    throw httpError(
      400,
      `${targetedMerchant.name} is not eligible for the selected delivery market`
    );
  }
  if (targetedMerchant) {
    let products = await searchMerchant(targetedMerchant.slug, normalizedQuery, {
      limit: merchantLimit,
      baseUrl,
      fetchImpl,
    });
    ...
    if (!products.length && fallbackQueries.length > 1) {
      ...
      queriesTried = [normalizedQuery, ...fallbackQueries];
      for (const fallbackQuery of fallbackQueries) {
        const result = await searchMerchant(targetedMerchant.slug, fallbackQuery, {
          limit: merchantLimit,
          baseUrl,
        ...
        if (products.length >= merchantLimit) break;
      }
    }
    products.sort((left, right) => {
      if (left.available !== right.available) return left.available ? -1 : 1;
    ...
    return {
      ...
      searchStrategy,
      queriesTried,
      source: "live_selected_merchant_ucp",
      selectedMerchant: targetedMerchant.slug,
      market: normalizedMarket || targetedMerchant.market,
      markets: [targetedMerchant.market],
      sort: "availability_then_lowest_native_price",
      products: pageProducts,
      merchants: [{
        merchant: targetedMerchant.slug,
        merchantName: targetedMerchant.name,
        market: targetedMerchant.market,
        currency: targetedMerchant.currency,
        status: "ok",
  ...
  const merchants = globalSearch.markets.map((entry) => ({
    ...
    currency: preferredCurrencyForMarket(entry.market),
    ...entry,
  }));
...
function checkoutInput({ selections, buyer, destination }) {
  const first = selections[0].selection;
  const checkout = {
    currency: first.currency || "INR",
    line_items: selections.map(({ selection, count }) => ({
      ...
      (group?.options || []).map((option) => {
        ...
        return {
          ...
          earliestFulfillmentTime:
            option.earliest_fulfillment_time || option.earliestFulfillmentTime || null,
          latestFulfillmentTime:
            option.latest_fulfillment_time || option.latestFulfillmentTime || null,
...
async function createCheckout(
  ...
  return createCheckoutFromSelections(
    [{ selectionToken, quantity }],
    { baseUrl, buyer, destination, deliveryCountry, fetchImpl }
  );
}

async function createCheckoutFromSelections(
  items,
  {
    baseUrl,
    buyer = null,
  ...
  const selections = (Array.isArray(items) ? items : []).map((item) => ({
    selection: verifySelection(item?.selectionToken),
    count: Math.min(Math.max(Math.floor(Number(item?.quantity) || 1), 1), 20),
  }));
  if (!selections.length) throw httpError(400, "At least one cart item is required");
  if (deliveryCountry) {
    const normalizedDeliveryCountry = normalizeCountryMarket(
      deliveryCountry,
      "deliveryCountry"
    );
    const staleSelection = selections.find(({ selection }) =>
      String(selection.deliveryMarket || "").toUpperCase()
        !== normalizedDeliveryCountry
    );
    if (staleSelection) {
      throw httpError(
        409,
        `The cart was searched for ${staleSelection.selection.deliveryMarket || "another country"}. Search again for delivery to ${normalizedDeliveryCountry}.`
      );
    }
  }
  const firstSelection = selections[0].selection;
  if (selections.some(({ selection }) => selection.merchant !== firstSelection.merchant)) {
    throw httpError(400, "A UCP checkout can contain products from only one merchant");
  }
  const selected = await discoverSelectionMerchant(firstSelection, fetchImpl);
  ...
  let payload = await callMcp({
    ...
    arguments: {
      checkout: checkoutInput({ selections, buyer, destination }),
    },
    baseUrl,
    fetchImpl,
  });
  ...
  const pricing = checkoutPricing(
    ...
    selections.reduce(
      (sum, { selection, count }) =>
        sum + Number(selection.priceMinor || 0) * count,
      0
    )
  );
  const checkout = checkoutResource(payload);
  ...
  return {
    ...
    productName: firstSelection.productName,
    variantName: firstSelection.variantName,
    variantId: firstSelection.variantId,
    quantity: selections.reduce((sum, item) => sum + item.count, 0),
    items: selections.map(({ selection, count }) => ({
    ...
    market: firstSelection.deliveryMarket || selected.market,
    currency: String(checkout?.currency || firstSelection.currency || "INR").toUpperCase(),
    quotedUnitPrice: Number(firstSelection.priceMinor || 0) / 100,
    ...pricing,
