  ...
  async () => {
    ...
    try {
      ...
      global.fetch = async (url) => {
        ...
        if (requests.length === 1) return Response.json({ mandates: [] });
        return Response.json({ data: { mandates: [{
          mandate_id: "mdt_wrapped",
          status: "ACTIVE",
          state: "AVAILABLE",
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(mandates[0].id, "mdt_unfiltered");
      assert.doesNotMatch(requests[3], /standing_only/);
    } finally {
      global.fetch = originalFetch;
      for (const name of envNames) {
        if (originalEnv[name] === undefined) delete process.env[name];
...
test(
  "Prava mandate charge retries a Visa cryptogram failure with the documented minimal body",
  { concurrency: false },
  async () => {
    const envNames = ["PRAVA_PUBLISHABLE_KEY", "PRAVA_SECRET_KEY", "PRAVA_API_BASE_URL"];
    const originalEnv = Object.fromEntries(
    ...
    try {
      ...
      global.fetch = async (_url, options = {}) => {
        ...
        if (bodies.length === 1) {
          return new Response(
            JSON.stringify({ error: { code: "VISA_400", message: "Fetching cryptogram failed" } }),
      ...
      const charge = await chargeMandate({
        ...
        purchaseContext: [{ merchant_details: { name: "Example" } }],
      });
      assert.equal(charge.transactionId, "txn_retry");
      assert.equal(bodies.length, 2);
      assert.ok(Array.isArray(bodies[0].purchase_context));
...
test(
  "Prava payment sessions repair empty descriptions and reconcile merchant charges",
  { concurrency: false },
  async () => {
    const originalPublishable = process.env.PRAVA_PUBLISHABLE_KEY;
    const originalSecret = process.env.PRAVA_SECRET_KEY;
    const originalBase = process.env.PRAVA_API_BASE_URL;
    const originalFetch = global.fetch;
    let body;
    try {
      process.env.PRAVA_PUBLISHABLE_KEY = "pk_test_example";
      ...
      global.fetch = async (_url, options = {}) => {
        ...
        return new Response(JSON.stringify({
          session_id: "ses_ucp_123",
          iframe_url: "https://checkout.sandbox.prava.space/s/ses_ucp_123",
        }), { status: 201, headers: { "content-type": "application/json" } });
      };
      ...
      assert.deepEqual(body.purchase_context[0].product_details, [{
        description: "Himalaya Wellness merchant quote total including charges",
        unit_price: "171.00",
        quantity: 1,
      }]);
    } finally {
      global.fetch = originalFetch;
      if (originalPublishable === undefined) delete process.env.PRAVA_PUBLISHABLE_KEY;
      else process.env.PRAVA_PUBLISHABLE_KEY = originalPublishable;
      if (originalSecret === undefined) delete process.env.PRAVA_SECRET_KEY;
      else process.env.PRAVA_SECRET_KEY = originalSecret;
      if (originalBase === undefined) delete process.env.PRAVA_API_BASE_URL;
      else process.env.PRAVA_API_BASE_URL = originalBase;
    }
  }
);
