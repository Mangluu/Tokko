...
function phoneDigits(countryCode, value) { return normalizeLocalPhone(countryCode, value).slice(0, 10); }
...
function FamilyEditor({ state, displayName, submitLabel, onSaved }) {
  ...
  const submit = async () => {
    ...
    if (!ownerLocal) return setStatus({ type: 'error', message: 'Add your phone number so Tokko can reach you.' });
    ...
    if (phones.includes(toE164(ownerPhone.countryCode, ownerLocal))) return setStatus({ type: 'error', message: 'Your phone and a family member cannot share a number.' });
    setBusy(true); setStatus({ message: 'Saving your family circle…' });
    try {
      const next = await api('/api/onboarding/profile', { method: 'PUT', body: { primaryParentName: ownerName.trim(), primaryParentCountryCode: ownerPhone.countryCode, primaryParentLocalPhone: ownerLocal, dependents: members.map(memberPayload) } });
      setStatus({ type: 'success', message: 'Family circle saved.' }); onSaved(next);
    } catch (error) { setStatus({ type: 'error', message: error.message }); }
    finally { setBusy(false); }
  };
  return <>
    <div className="tf-card tf-owner-card"><span className="tf-card-icon"><Fingerprint size={20} /></span><label><span>What should Tokko call you?</span><input value={ownerName} onChange={(event) => setOwnerName(event.target.value)} placeholder="Your name" /></label><small>{state.account?.email} · secure account owner</small><label className="tf-phone-field"><span>Your phone number</span><span className="tf-phone-input"><select aria-label="Your country calling code" value={ownerPhone.countryCode} onChange={(event) => setOwnerPhone({ ...ownerPhone, countryCode: event.target.value })}>{COUNTRY_CODES.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><input inputMode="tel" autoComplete="tel" value={ownerPhone.localPhone} onChange={(event) => setOwnerPhone({ ...ownerPhone, localPhone: phoneDigits(ownerPhone.countryCode, event.target.value) })} placeholder="98765 43210" required /></span></label></div>
    <div className="tf-section-heading"><div><h2>Your care circle <em>optional</em></h2><p>The people who can message Tokko for help. You can add them anytime.</p></div><button className="tf-small-button" type="button" onClick={() => setEditing({ index: undefined, member: null })}><Plus size={16} /> Add</button></div>
...
function blankAddress(state) {
  // No prefilled contact — tapping a member below sets them as the delivery contact.
  return { label: 'Home', addressLine1: '', addressLine2: '', city: '', state: '', postalCode: '', countryCode: 'IN', contactName: '', contactCountryCode: '+91', contactLocalPhone: '', memberIds: (state.profile?.dependents || []).map((member) => String(member.id)) };
}
...
function App() {
  ...
  if (page === 'spending') return <SetupShell stage={3} eyebrow="Payment" title="Add a card for easy checkout." copy="Tokko always asks you before ordering. A saved card lets approved orders check out instantly — you can also add one later from Wallet." theme={theme} onTheme={toggleTheme} onExit={() => setPage('landing')}><SpendingManager state={state} onState={setState} onboarding onContinue={(next) => { setState(next); setPage('dashboard'); }} /></SetupShell>;
