      ...
      <div className="shopper-message-content">
        ...
        {!!message.productChoices?.length && (
          <section className="ucp-product-results" aria-label="UCP product choices">
            ...
            <div className="ucp-product-grid">
              {message.productChoices.map((product, index) => (
                <article className={`ucp-product-card ${!product.available ? 'unavailable' : ''}`} key={`${product.selectionToken}-${index}`}>
                  ...
                  >
                    {product.available ? 'Select & checkout' : 'Unavailable'}
                  </button>
                </article>
              ))}
            </div>
            {message.productPagination?.hasMore && (
              ...
              >
                Show 50 more <ArrowRight size={14} />
              </button>
            )}
          </section>
        )}
          ...
          <section className="shopper-checkout-breakdown" aria-label="Merchant checkout total">
            ...
            <small>
              {message.checkoutSummary.shippingQuoted
                ...
                : 'The merchant has not returned a shipping charge yet; confirm it on the merchant checkout.'}
            </small>
          </section>
        )}
        {message.nextAction?.url && (
          <section className="shopper-payment-action" aria-label="Payment action">
            ...
            {message.nextAction.type === 'prava_card_approval' && (
              <button
                type="button"
...
function ShopperPage({ setup, onZeptoReconnected }) {
  const initialContactPhone = phoneParts(setup.profile.phone);
  ...
  const selectShopperAddress = async (address) => {
    ...
    try {
      const result = await api('/api/addresses/select', {
        method: 'POST',
        body: { addressId },
      });
      setSelectedAddress(addressId);
      setAddressChooserOpen(false);
      setAddressStatus(`${address.label || 'Delivery address'} selected. Tokko is ready to shop.`);
      ...
      setMessages((current) => [
        ...current,
        {
          ...
          role: 'assistant',
          content: `delivery address confirmed: ${readable}\n\nask me anything for shopping.`,
        },
      ]);
    } catch (error) {
      setSelectedAddress('');
  ...
  const saveShopperAddress = async (event) => {
    ...
    if (
      ...
      setAddressStatus('Enter a valid 10-digit Indian delivery mobile number.');
      return;
    }
    setAddressBusy(true);
    ...
    try {
      ...
      const saved = await api('/api/addresses', {
        ...
        body: {
          ...
          shortAddress: addressForm.shortAddress.trim(),
          contactName: addressForm.contactName.trim(),
          contactNumber: toE164(addressForm.contactCountryCode, localPhone),
        },
      });
  ...
  const continuePayment = async (paymentMessage) => {
    ...
    try {
      const result = await api('/api/hermes/checkout/continue', {
        method: 'POST',
        body: { checkoutId },
      });
      appendHermesResult(result, waitingId);
  ...
  const selectUcpProduct = async (product) => {
    ...
    setBusy(true);
    setStatus(`Creating a secure ${product.merchantName} checkout…`);
    try {
      const result = await api('/api/merchants/ucp/checkout', {
        method: 'POST',
        body: { selectionToken: product.selectionToken, quantity: 1 },
      });
      const checkedMandates = Number(result.mandateCheck?.checkedMandateCount || 0);
      const mandateUsed = result.paymentRoute === 'mandate' && result.paymentHandoff?.credentials;
      const savedCardUsed = result.paymentRoute === 'prava_card' && result.savedCard;
      const cardSelectionRequired = result.paymentRoute === 'card_selection_required';
      const merchantHandoffUrl = result.merchantHandoffUrl || result.continueUrl || result.checkoutUrl;
      appendHermesResult({
        message: cardSelectionRequired
          ? `your ${product.merchantName} checkout has the selected address and phone prefilled. no active mandate covers ${result.currency} ${result.totalAmount}. choose a saved prava card below.`
          : mandateUsed
          ? `your ${product.merchantName} checkout is ready with the selected address and phone prefilled. i checked ${checkedMandates} prava mandates and selected an eligible mandate for ${result.currency} ${result.totalAmount}.`
          : savedCardUsed
            ? `your ${product.merchantName} checkout is ready with the selected address and phone prefilled. no eligible mandate covered the total, so tokko selected your saved ${result.savedCard.brand} ending ${result.savedCard.last4}. the merchant checkout does not advertise a compatible prava payment handler, so it may still ask you to confirm payment.`
            : `your ${product.merchantName} checkout is ready with the selected address and phone prefilled. i checked ${checkedMandates} prava mandates, but no eligible mandate or saved card could cover ${result.currency} ${result.totalAmount}.`,
        tools: [{
        ...
        checkoutSummary: {
          ...
          shippingQuoted: result.shippingQuoted === true,
          destinationSelected: result.destinationSelected === true,
          phoneAccepted: result.phoneAccepted === true,
          reconciles: result.reconciles,
        },
        nextAction: cardSelectionRequired ? null : {
            type: 'merchant_ucp_checkout',
            url: merchantHandoffUrl,
            label: `Continue to ${result.merchantName} checkout`,
            paymentHandoff: result.paymentHandoff || null,
            paymentSelection: result.paymentSelection || null,
            checkoutSummary: {
              currency: result.currency,
              totalAmount: result.totalAmount,
              totals: result.totals || [],
  ...
  const showMoreProducts = async (sourceMessage) => {
    ...
    const offset = Number(sourceMessage.productPagination?.nextOffset || 50);
    ...
    setBusy(true);
    setStatus('Loading 50 more market-sorted matches…');
    try {
      const result = await api('/api/merchants/ucp/search', {
        method: 'POST',
        body: { query, limit: 50, offset },
      });
      appendHermesResult({
    ...
    <div className="subpage shopper-page">
      <section className="shopper-shell">
        <header className="shopper-header">
          <div className="shopper-identity">
            ...
            <div>
              ...
              <span>Compare live wellness catalogues, select a product, and continue on the merchant’s secure checkout.</span>
            </div>
          </div>
          ...
          <div className="shopper-address-heading">
            ...
            <div>
              ...
              <small>{selectedAddress && !addressChooserOpen
                ? confirmedAddress?.formattedAddress || confirmedAddress?.shortAddress || 'Address selected for this shopping session.'
                    ...
                    return (
                      <label className={checked ? 'selected' : ''} key={addressId || `shopper-address-${index}`}>
                        ...
                        <span>
                          ...
                          <small>{address.formattedAddress || address.shortAddress || 'Saved Tokko address'}</small>
                        </span>
        ...
        <form className="shopper-composer" onSubmit={submit}>
          ...
          <button type="submit" className="shopper-send-button" disabled={busy || transcribing || !selectedAddress || !draft.trim()} aria-label="Send message">
            <ArrowRight size={19} />
          </button>
        </form>
        {listening && (
            ...
            <form className="address-form-fields" onSubmit={saveShopperAddress}>
              <div className="form-grid two-columns">
                ...
                <div className="field-group">
                  <span>Delivery contact phone</span>
                  <div className="phone-input-row">
                    <select aria-label="Delivery contact country code" value={addressForm.contactCountryCode} onChange={(event) => updateShopperAddressForm('contactCountryCode', event.target.value)}><option value="+91">IN +91</option></select>
  ...
  return (
    <div className="subpage connectors-page">
      ...
      <div className="support-callout connector-support-note">
        <LockKeyhole size={18} />
        <div><strong>Merchant-hosted payment</strong><span>Tokko creates the selected merchant checkout, then opens its exact UCP continue URL for shipping and payment.</span></div>
      </div>
    </div>
  );
}
...
function MandatesPage({ setup, setSetup, onAddCard }) {
  ...
  const createMandate = async () => {
    ...
    try {
      ...
      setPendingSession(session);
      setStatus(
        'Mandate created. Open Prava once to approve it with your passkey, then return and refresh status.'
      );
    } catch (error) {
  ...
  return (
    <div className="subpage mandate-management-page">
      ...
      {mandateTab === 'current' && <section className="dashboard-section mandate-create-card">
        ...
        {pendingSession?.approvalUrl && (
          <div className="mandate-approval-callout">
            <div><strong>Approval required once</strong><span>Complete the Prava passkey approval, then refresh the mandate status.</span></div>
            <a className="button primary-button" href={pendingSession.approvalUrl} target="_blank" rel="noreferrer"><ExternalLink size={16} /> Open Prava approval</a>
          </div>
        )}
        {status && <p className={`form-message ${/could not|invalid|required|error/i.test(status) ? 'error-message' : ''}`} role="status">{status}</p>}
      </section>}
...
function Dashboard({ setup, setSetup, page, setPage, onLogout, onConnectZepto }) {
    ...
    <main className={`dashboard-page ${activePage === 'assistant' ? 'assistant-mode' : ''}`}>
      ...
      <section className={`dashboard-main ${activePage === 'assistant' ? 'assistant-main' : ''}`}>
        ...
        {activePage === 'overview' ? <DashboardOverview setup={setup} setSetup={setSetup} setPage={setPage} orders={orders} ordersBusy={ordersBusy} ordersStatus={ordersStatus} checkoutFlows={checkoutFlows} checkoutStatus={checkoutStatus} onRefreshOrders={refreshOrders} onConnectZepto={onConnectZepto} /> : activePage === 'connectors' ? (
          ...
          <MandatesPage setup={setup} setSetup={setSetup} onAddCard={() => setPage('card')} />
        ) : activePage === 'alerts' ? (
          <AlertsPage
            setup={setup}
            orders={orders}
...
function App() {
  ...
  useEffect(() => {
    ...
    if (returningFromPravaCard) {
      setView('dashboard');
      setDashboardPage('card');
    ...
    api('/api/me')
      .then((state) => {
        ...
        if (
          ...
          (returningFromPravaCard || ['login', 'entry'].includes(view))
        ) {
          setView('dashboard');
        }
      })
  ...
  if (view === 'entry') {
    ...
    content = <Dashboard setup={setup} setSetup={setSetup} page={dashboardPage} setPage={setDashboardPage} onLogout={logout} onConnectZepto={goToZepto} />;
  } else {
