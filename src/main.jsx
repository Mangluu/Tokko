...
function orderItems(source) {
  ...
  return raw.map((it) => ({ name: it.name || it.productName || it.label || 'Item', quantity: Number(it.quantity) || null }));
}
function OrdersView({ state }) {
  const [orders, setOrders] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(null); const [openId, setOpenId] = useState(null);
  useEffect(() => { (async () => {
    try {
      let list = [];
      try { const result = await api('/api/merchants/ucp/orders?limit=50'); list = (result.orders || []).map((o) => ({ id: o.orderId, merchantName: o.merchantName || o.merchant || 'Order', amount: o.totalAmount != null ? Number(o.totalAmount) : null, currency: o.currency || 'INR', status: o.status || null, when: o.createdAt || null, items: orderItems(o.quote?.items || o.quote?.lineItems || o.quote?.products || o.quote?.cartItems), trackingUrl: o.merchantOrder?.url || null, paymentRoute: o.paymentRoute || null, receiptRef: o.paymentReference?.fingerprint || o.prava?.transactionId || null })); } catch (_) { /* order-intent endpoint absent on this backend, fall back below */ }
      if (!list.length) { const result = await api('/api/checkout/activity?limit=50'); list = (result.checkoutFlows || []).filter(Boolean).map((o) => ({ id: o.id, merchantName: o.merchantName || o.platform || 'Order', amount: o.pravaCharge?.amount != null ? Number(o.pravaCharge.amount) : null, currency: o.pravaCharge?.currency || 'INR', status: o.pravaCharge?.status || o.status || null, when: o.pravaCharge?.reportedAt || null, items: orderItems(o.cartItems), trackingUrl: null, paymentRoute: o.paymentRoute || null, receiptRef: o.pravaCharge?.reference || null })); }
      setOrders(list);
    } catch (err) { setError(err.message); } finally { setLoading(false); }
  })(); }, []);
  const completed = orders.filter((o) => ['approved', 'completed', 'confirmed', 'paid', 'fulfilled'].includes(String(o.status || '').toLowerCase()));
  const totalCharged = orders.reduce((sum, o) => sum + (Number(o.amount) || 0), 0);
  return <><header className="tf-view-title"><span className="tf-view-icon is-plum"><ShoppingBag /></span><div><h1>Orders & invoices</h1><p>Every order Tokko has placed for your family, with amounts and receipts.</p></div></header><div className="tf-analytics-grid"><div><ShoppingBag size={19} /><strong>{orders.length}</strong><span>Orders placed</span></div><div><CheckCircle2 size={19} /><strong>{completed.length}</strong><span>Completed</span></div><div><CircleDollarSign size={19} /><strong>{money(totalCharged)}</strong><span>Total charged</span></div></div>{loading ? <EmptyState icon={ShoppingBag} title="Loading orders" copy="Fetching your family’s order history." /> : error ? <Status value={{ type: 'error', message: error }} /> : orders.length ? <div className="tf-order-list">{orders.map((order) => { const open = openId === order.id; return <article className={`tf-order-card ${open ? 'is-open' : ''}`} key={order.id}><button type="button" className="tf-order-head" onClick={() => setOpenId(open ? null : order.id)}><span className="tf-card-icon"><Receipt size={18} /></span><div><strong>{order.merchantName}</strong><small>{order.when ? new Date(order.when).toLocaleDateString() : (order.status || 'in progress')}{order.items.length ? ` · ${order.items.length} item${order.items.length === 1 ? '' : 's'}` : ''}</small></div><span className="tf-order-amount">{order.amount != null ? money(order.amount, order.currency) : '—'}</span><ChevronRight size={16} className="tf-order-caret" /></button>{open && <div className="tf-order-detail">{order.items.length > 0 && <ul className="tf-order-items">{order.items.map((item, index) => <li key={index}><span>{item.name}</span><em>{item.quantity ? `× ${item.quantity}` : ''}</em></li>)}</ul>}<dl className="tf-order-meta"><div><dt>Status</dt><dd>{order.status || '—'}</dd></div>{order.paymentRoute && <div><dt>Route</dt><dd>{String(order.paymentRoute).replace(/_/g, ' ')}</dd></div>}{order.receiptRef && <div><dt>Receipt</dt><dd className="tf-order-ref">{order.receiptRef}</dd></div>}{order.trackingUrl && <div><dt>Track</dt><dd><a href={order.trackingUrl} target="_blank" rel="noreferrer">Open order</a></dd></div>}</dl></div>}</article>; })}</div> : <EmptyState icon={ShoppingBag} title="No orders yet" copy="When Tokko places an approved order, it appears here with its invoice." />}</>;
}
