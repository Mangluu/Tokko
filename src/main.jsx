...
import {
  ...
  Plus, RefreshCw, Settings, ShieldCheck, Sparkles, Sun, Trash2, Users, X,
} from 'lucide-react';
...
function AddressDialog({ open, state, initial, onClose, onSaved }) {
  ...
  const toggleMember = (id) => setAddress((current) => ({ ...current, memberIds: current.memberIds.includes(String(id)) ? current.memberIds.filter((item) => item !== String(id)) : [...current.memberIds, String(id)] }));
  const submit = async (event) => {
...
function SetupShell({ stage, eyebrow, title, copy, children, theme, onTheme, onExit }) {
  return <section className="tf-setup-screen"><SetupHeader stage={stage} theme={theme} onTheme={onTheme} onExit={onExit} /><main className="tf-setup-main"><header><span className="tokko-panel-kicker"><Sparkles size={15} /> {eyebrow}</span><h1>{title}</h1><p>{copy}</p></header>{children}</main><aside className="tf-ai-rail"><span className="tf-orbit"><i /><Sparkles size={20} /></span><small>Tokko is learning your family’s boundaries</small><strong>{stage === 1 ? 'Simple for them. Clear for you.' : stage === 2 ? 'The right care, to the right door.' : 'Quiet automation. Visible control.'}</strong><p>{stage === 1 ? 'Each person messages from a familiar number. Tokko keeps their context separate.' : stage === 2 ? 'A delivery address is chosen before shopping, so availability and totals stay truthful.' : 'Rules decide what can flow. Ambiguity always comes back to a human.'}</p></aside></section>;
}
